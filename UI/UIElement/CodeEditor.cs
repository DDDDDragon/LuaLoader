...
{
    public class CodeEditor : UIPanel
    {
        ...
        public CodeEditor(string text = "", Point cursorPosition = default(Point), Color color = default(Color), Vector2 symSizeOffice = default)
        {
            _text = text;
            ...
            var c = LuaLoader.DefaultFont.MeasureString(cursorSym[0].ToString());
            symHitBox.Width = (int)(c.X + symSizeOffice.X);
            symHitBox.Height = (int)(c.Y + symSizeOffice.Y);
            symOffsetX = (c.X + symSizeOffice.X) / 2f;
        }
        ...
        public override void OnInitialization()
        {
            ...
            up = new Utils.KeyCooldown(() => Main.keyState.IsKeyDown(Keys.Up));
            down = new Utils.KeyCooldown(() => Main.keyState.IsKeyDown(Keys.Down));
            left = new Utils.KeyCooldown(() => Main.keyState.IsKeyDown(Keys.Left));
            right = new Utils.KeyCooldown(() => Main.keyState.IsKeyDown(Keys.Right));
            enter = new Utils.KeyCooldown(() => Main.keyState.IsKeyDown(Keys.Enter));
        ...
        protected override void DrawChildren(SpriteBatch sb)
        {
            ...
            for (int i = 0; i < texts.Length; i++)
            {
                ...
                if (isEnableIME && i == CursorPosition.Y && (j <= 8 && j >= 0))
                {
                    ...
                    sb.DrawString(LuaLoader.DefaultFont, new StringBuilder(cursorSym), Info.Location + new Vector2(x - symOffsetX, offsetY) + offset, _color);
                }
                sb.DrawString(LuaLoader.DefaultFont, new StringBuilder(text), Info.Location + new Vector2(0f, offsetY) + offset, _color);
                offsetY += LuaLoader.DynamicSpriteFontInfoManager.GetCharacterSize(FontAssets.MouseText.Value, '啊').Y;
            }
